...
public interface SqlQuery extends SqlFluent<SqlQuery> {
	 ...
	 * 検索結果の取得（終端処理）
	 *
	 * @return 検索結果のリスト. MapのキーはCamel式に変換される.
	 * @throws SQLException SQL例外
	 *
	 ...
	 * @see SqlAgent#query(SqlContext, boolean)
	 */
	List<Map<String, Object>> collect(boolean toCamel) throws SQLException;
	 ...
	 * @throws SQLException SQL例外
	 */
	Map<String, Object> first(boolean toCamel) throws DataNotFoundException, SQLException;

	/**
	 * 検索結果の先頭行を取得（終端処理）
	 *
	 * @return 検索結果の先頭行をMapに詰めたもの. MapのキーはCamel式に変換される.
	 * @throws DataNotFoundException 検索結果が０件の場合
	 * @throws SQLException SQL例外
	 */
