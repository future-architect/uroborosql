...
public class ListCommandTest extends ReaderTestSupport {
	...
	void testExecute() throws Exception {
		...
		var flag = command.execute(reader, "list example".split("\\s+"), sqlConfig, new Properties());
		assertTrue(flag);
		assertConsoleOutputContains("example/insert_product");
		assertConsoleOutputNotContains("ddl/create_tables");

		command.execute(reader, "list".split("\\s+"), sqlConfig, new Properties());
		assertConsoleOutputContains("example/insert_product");
		assertConsoleOutputContains("ddl/create_tables");
	}
	...
	void testShowHelp() throws Exception {
		...
		reader.flush();
		assertConsoleOutputContains("list loaded sql files.");
	}
	...
	void testUtilityMethods() throws Exception {
		...
		assertThat(command.match("List"), is(true));
		assertThat(command.match("NoMatchCommand"), is(false));
		assertThat(command.is("list"), is(true));
