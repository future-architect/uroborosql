...
public class QueryCacheTest {
	...
	public static void setUpBeforeClass() throws Exception {
		String url = "jdbc:h2:mem:" + QueryCacheTest.class.getSimpleName() + ";DB_CLOSE_DELAY=-1";
		String user = null;
		...
		config = UroboroSQL.builder(url, user, password)
				.setSqlAgentFactory(new SqlAgentFactoryImpl().setUseEntityQueryCache(true))
				.build();
	}
	...
	@Test
	public void testInsertAndFindMultiKey() throws Exception {
		...
		try (SqlAgent agent = config.agent()) {
			agent.required(() -> {
				...
				TestDataMultiKeyEntity data = agent.find(TestDataMultiKeyEntity.class, 1, "key1").orElse(null);
				assertThat(data, is(test1));
				data = agent.find(TestDataMultiKeyEntity.class, 1, "key2").orElse(null);
				assertThat(data, is(test2));
				data = agent.find(TestDataMultiKeyEntity.class, 2, "key1").orElse(null);
				assertThat(data, is(test3));
				data = agent.find(TestDataMultiKeyEntity.class, 2, "key2").orElse(null);
				assertThat(data, is(nullValue()));
