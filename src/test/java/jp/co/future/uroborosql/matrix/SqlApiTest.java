...
class SqlApiTest extends AbstractMatrixTest {
	...
	void testBatchInsertWithAutoGeneratedKeys() throws Exception {
		...
		var products = IntStream.rangeClosed(1, 3)
				.mapToObj(i -> {
					var product = new Product();
		...
		// バッチインサート実行
		var insertCount = agent.inserts(Product.class, products.stream());
		assertThat(insertCount, is(3));
		
		// データが正しく挿入されたことを確認
		var insertedProducts = agent.query(Product.class)
				.in("productId", List.of(1, 2, 3))
				.collect();
	...
	void testBatchInsertAndReturnWithAutoGeneratedKeys() throws Exception {
		...
		// テスト用のProductエンティティを作成
		var products = IntStream.rangeClosed(11, 13)
				.mapToObj(i -> {
					var product = new Product();
					product.setProductId(i);
					product.setProductName("商品" + i);
		...
		var returnedProducts = agent.insertsAndReturn(Product.class, products.stream()).collect(Collectors.toList());
		assertThat(returnedProducts.size(), is(3));
		
		// データが正しく挿入されたことを確認
		var insertedProducts = agent.query(Product.class)
				.in("productId", List.of(11, 12, 13))
				.collect();
